"""Designer API Router - Agent emotional profiles and relationship configs

Agents: stored in SQLite (agents table, emotional_profile column)
Relationships: stored in JSON files (configs/relationships/*.json)
...
from services.config_loader import clear_config_cache

router = APIRouter(prefix="/api/designer", tags=["designer"])

# Config paths (for relationships and moods - still JSON)
CONFIGS_DIR = Path(__file__).parent.parent.parent / "configs"
RELATIONSHIPS_DIR = CONFIGS_DIR / "relationships"
MOODS_FILE = CONFIGS_DIR / "moods.json"
...
async def list_moods() -> dict[str, Any]:
    ...
    data = load_json(MOODS_FILE)
    moods = data.get("moods", {})
    
    return {
        "moods": [
            {"id": mood_id, **mood_data}
...
async def list_relationships() -> list[dict]:
    ...
    if RELATIONSHIPS_DIR.exists():
        for path in sorted(RELATIONSHIPS_DIR.glob("*.json")):
            try:
                data = load_json(path)
                relationships.append({
                    "type": path.stem,
                    "description": data.get("description", ""),
                    "trigger_count": len(data.get("trigger_mood_map", {})),
                })
...
async def get_relationship(rel_type: str) -> dict:
    ...
    path = RELATIONSHIPS_DIR / f"{rel_type}.json"
...
async def update_relationship(rel_type: str, config: dict[str, Any]) -> dict:
    ...
    clear_config_cache()  # Relationships still use cache
    return existing
...
async def create_relationship(rel_type: str, config: dict[str, Any]) -> dict:
    ...
    if "type" not in config:
        config["type"] = rel_type
    if "description" not in config:
        config["description"] = ""
    if "trigger_mood_map" not in config:
        config["trigger_mood_map"] = {}
