...
Use `GET /archetypes` to fetch the live list. Current backend-defined set:

- `aggressive` (`Aggressive`): Demanding, critical, impatient user.
  - Good for testing: negative-trigger resilience, trust loss behavior, conflict handling.
- `supportive` (`Supportive`): Encouraging, grateful, empathetic user.
  - Good for testing: trust gain speed, positive bonding, intimacy growth.
- `playful` (`Playful`): Joking, teasing, game-oriented user.
  - Good for testing: banter/teasing calibration, playfulness safety.
- `flirty` (`Flirty`): Romantic, intimate, affectionate user.
  - Good for testing: flirting response direction, intimacy/attachment ramps.
- `neutral` (`Neutral`): Everyday conversation, tasks, small talk.
  - Good for testing: baseline stability under normal traffic.
- `random` (`Random`): Unpredictable mix of all behaviors.
  - Good for testing: robustness across mixed trigger distributions.
- `rough_day_then_recover` (`Rough Day -> Recovery`): Starts critical/tense, then recovers.
  - Good for testing: trust loss/recovery cycle, resilience after early conflict.
- `lonely_then_playful` (`Lonely -> Playful`): Starts vulnerable/withdrawn, then becomes warm/playful.
  - Good for testing: vulnerability-to-play transition, reconnection behavior.
- `moody_week` (`Moody Week`): Alternates negative/neutral/positive phases.
  - Good for testing: volatility handling and mood recovery consistency across phase shifts.
