...
Updated: 2026-02-03 (Backend access control + DB routing)

## Update: Multi-Agent Routing (Current)
**Current model:** Routing is DB-backed and gated by user access checks.
- Requests include `X-User-Id` and `X-Agent-Id`
- Backend verifies access via `user_agents` and `session_participants`
- Agent routing uses the stored `clawdbot_agent_id`

This replaces the earlier single-agent hardening (see historical note below).

## Historical Note: Cross-agent routing to Beatrice ("main")
**What happened:** The backend could accidentally route requests to Beatrice if a single-agent env var was missing/mis-set.  
**Status:** Superseded by DB-backed routing and per-user access control.

## Hardening: CORS
**Current config:** Allowlist origins via `ALLOWED_ORIGINS`, but `allow_methods=["*"]` and `allow_headers=["*"]`.  
**Recommendation:** Tighten methods/headers if the service is exposed beyond the local network.
...
## Hardening: Access Control (2026-02-03)
- `/api/agents/{id}`, `/api/sessions/{id}`, and `/api/memory*` now require `X-User-Id`
- Access checks enforce `user_agents` and `session_participants`
