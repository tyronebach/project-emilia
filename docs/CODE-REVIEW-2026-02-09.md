...
### C3. Emotional state read-modify-write race condition — RESOLVED
...
**Fix**: Per-user-agent `threading.Lock` with double-checked locking serializes read-modify-write sections.
...
### H7. SSE stream buffer lost on connection drop — RESOLVED
**File**: `frontend/src/utils/api.ts:259-361`
Partial buffer silently discarded on server disconnect. UI stuck in "thinking" state.
**Fix**: Added `receivedDone` flag; after read loop ends without done event, flushes partial content via `onDone`.
...
### H9. No V2 test coverage — RESOLVED
...
**Fix**: Added 38 new tests covering all V2 components (80 total tests, up from 42).
...
### H13. User message stored before LLM call can orphan — RESOLVED
**File**: `backend/routers/chat.py:519, 613`
LLM failure leaves user message in DB with no assistant response.
**Fix**: Both streaming and non-streaming paths now wrap LLM call in try/except and delete orphaned user message on failure.
...
### H15. API.md missing 20 endpoints; AGENTS.md lists deleted files — RESOLVED
**Files**: `docs/API.md`, `AGENTS.md`
Emotional (6) + designer_v2 (14) endpoints undocumented. AGENTS.md still has clawdbot.py, stt.py.
**Fix**: Added all 20 endpoints to API.md. Updated AGENTS.md routers, services, and DB schema.
...
| # | Area | Finding | Status |
|---|------|---------|--------|
| M1 | Backend | `_process_emotion_post_llm` sync I/O blocks event loop | RESOLVED |
| M2 | Backend | `config_loader` lru_cache never invalidated on DB changes | RESOLVED (via M9) |
| M3 | Backend | Simulate endpoint intensity formula differs from actual engine | RESOLVED |
