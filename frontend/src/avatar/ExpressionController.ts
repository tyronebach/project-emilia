...
export class ExpressionController {
  ...
  setMood(mood: string, intensity: number = 1): void {
    console.log(`[ExpressionController] setMood called: ${mood} @ ${intensity}`);
    console.log(`[ExpressionController] expressionManager:`, !!this.expressionManager);

    if (!this.expressionManager) return;

    const vrmExpr = this._getVrmExpression(mood);
    if (!vrmExpr) {
      console.warn(`[ExpressionController] No expression found for: ${mood}`);
      console.log(`[ExpressionController] Available expressions:`, Array.from(this.availableExpressions));
      return;
    }

    intensity = Math.max(0, Math.min(1, intensity));

    console.log(`[ExpressionController] Mapping ${mood} -> ${vrmExpr} @ ${intensity}`);
