...
import { addAgentToSession, removeAgentFromSession } from '../../utils/api';
...
export default function ManageParticipantsPanel({ sessionId, onClose }: ManageParticipantsPanelProps) {
  const sessionAgents = useChatStore(state => state.sessionAgents);
  const addSessionAgent = useChatStore(state => state.addSessionAgent);
  const removeSessionAgent = useChatStore(state => state.removeSessionAgent);
  const setSessionAgents = useChatStore(state => state.setSessionAgents);
  ...
  const availableAgents = userAgents.filter(
    agent => !sessionAgents.some(sa => sa.id === agent.id)
  );
  ...
  const handleAddAgent = async (agent: Agent) => {
    ...
    setError(null);
    try {
      const updatedSession = await addAgentToSession(sessionId, agent.id);
      if (updatedSession.agents) {
        setSessionAgents(updatedSession.agents);
      } else {
        addSessionAgent(agent);
      }
    } catch (err) {
  ...
  const handleRemoveAgent = async (agentId: string) => {
    ...
    setError(null);
    try {
      const updatedSession = await removeAgentFromSession(sessionId, agentId);
      if (updatedSession.agents) {
        setSessionAgents(updatedSession.agents);
      } else {
        removeSessionAgent(agentId);
      }
    } catch (err) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-80 bg-base-100 shadow-xl z-50 flex flex-col">
      ...
      <div className="flex items-center justify-between p-4 border-b border-base-300">
        <h2 className="text-lg font-bold">Participants ({sessionAgents.length})</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {availableAgents.length === 0 && sessionAgents.length === userAgents.length && (
